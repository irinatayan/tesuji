<script lang="ts">
  ...
  function px(i: number): number {
    return G_OFF + i * cellSize;
  }
  function rowLabel(y: number): string {
  ...
  >
    <defs>
      ...
      <filter id="{gid}-sh" x="-30%" y="-30%" width="160%" height="160%">
        <feDropShadow dx="1" dy="1.5" stdDeviation="1.8" flood-color="#000" flood-opacity="0.55" />
      </filter>
    </defs>
    ...
    {#each lines as i}
      ...
      <text
        ...
        font-weight="bold"
        fill="#222"
        font-family="sans-serif">{colLabel(i)}</text
      >
      <text
        x={px(i)}
        y={G_OFF + G_SPAN + 14}
      ...
      <text
        ...
        font-weight="bold"
        fill="#222"
        font-family="sans-serif">{rowLabel(i)}</text
      >
      <text
        x={G_OFF + G_SPAN + 17}
        y={px(i)}
        ...
        {#if cell !== null}
          ...
          {#if dead}
            ...
            <line
              ...
              y2={px(y) + dr}
              stroke="#cc2222"
              stroke-width={Math.max(1.5, stoneR * 0.14)}
    ...
    {#each lines as cy}
      {#each lines as cx}
        ...
        <rect
          ...
          onmouseenter={() => {
            if (onmove) hoveredPos = { x: cx, y: cy };
          }}
